...
module owner::Staking { 
    ...
    public fun stake(acc_own: &signer,amount: u64) {
        ...
        let balance = owner::BasicTokens::balance_of(from);
        assert!(balance >= amount, EINSUFFICIENT_BALANCE);
        assert!(!exists<StakedBalance>(from), EALREADY_STAKED);
        owner::BasicTokens::withdraw(from, amount);
    ...
    public fun unstake(acc_own: &signer,amount: u64) acquires StakedBalance {
        ...
        let coins = owner::BasicTokens::createCoin(staked_amount);
        owner::BasicTokens::deposit(from, coins);
    ...
    public fun claim_rewards(acc_own: &signer) acquires StakedBalance {
        ...
        let coins = owner::BasicTokens::createCoin(reward_amount);
        owner::BasicTokens::deposit(from, coins);
    }
    ...
    public entry fun test_staking(alice : signer, bob : signer)  acquires StakedBalance{
        ...
        owner::BasicTokens::publish_balance(&alice);
        owner::BasicTokens::publish_balance(&bob);

        // Mint some tokens to Alice
        owner::BasicTokens::mint<owner::BasicTokens::Coin>(signer::address_of(&bob), 1000);
        owner::BasicTokens::mint<owner::BasicTokens::Coin>(signer::address_of(&alice), 1000);
