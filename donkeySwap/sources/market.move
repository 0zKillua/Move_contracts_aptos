module donkeyswap::market {
    ...
    public fun add_liquidity<CoinType>(
        ...
        //@audit let's say attacker added a fulfillable limit order, but does not have quoteToken store registered so this will revert, which will revert all fulfilling orders, which will revert this add_liquidity call as well. so it becomes impossible to add liquidity for any user sicne fullfill orders is called automatically. LOL..
    ...
    //; asymmetry is found in market order and limit order. 
    //; ideally limit order(with min_quote=0) should be a market order giving same return amount, but no, limit order will give sligtly more tokens. 
    /// @audit Limit order users gain because their funds dont dilute the pools price until execution, giving them a better rate.
    // @audit limit_swap locks base, keeping base_size unchanged, while swap increases base_size, affecting calculations differently.

        /*for above bug
        User Impact:
        Limit Swap: Higher returns (e.g., 100 USDC) due to smaller base_size, higher price.

        General Swap: Lower returns (e.g., 90.909 USDC) due to larger base_size, lower price.

        Winner: Limit order users benefit, especially with small pools or min=0.
    ...
    public fun cancel_order<BaseCoinType>(
        ...
        //!excellent bug. user requested baseCoinType coins in return, of amount=order.base where as this order related coin may be different from user requested, in short user can place usdc limit order of 1000 tokens, cancel order speicifying BTC, so will recieve 1000 BTC in return? LOL. 
        deposit_funds<BaseCoinType>(order_store, address_of(user), order.base);

        drop_order(order_store, &order);
    }

//; execute all those orders that match Cointype as returning token. 
    public fun fulfill_orders<CoinType>(): vector<u64> acquires OrderStore, CoinStore {
        ...
        while (true) {
            //;gets the first order from orders list. 
    ...
    public entry fun admin_withdraw_protocol_fees<CoinType>(
        ...
        let order_store = borrow_global_mut<OrderStore>(@donkeyswap);
        //@audit unlocking will increase liquidity value, but coins are deposited into admins account. we should only update the locked value. 
        //@audit due to this unlock,The pool appears to have more liquidity than it actually holds.
        //@audit violating the invariant liquidity[CoinType] + locked[CoinType] == CoinStore<CoinType>.coins.balance.
    ...
    /*
    probabale fix:
    let current_locked = get_locked_size_internal(order_store, type_info::type_of<CoinType>());
    table::upsert<TypeInfo, u64>(&mut order_store.locked, type_info::type_of<CoinType>(), current_locked - fees);
    or
    delete coin::deposit,
    call deposit_funds after unlock and deposit into admin account. this will balance the liquidity. 
    */
    ...
    fun register_liquidity(
        ...
        let current_liquidity = get_liquidity_size_internal(order_store, type);
        //; update at the key=type to value=curr+size in this table. 
        table::upsert<TypeInfo, u64>(&mut order_store.liquidity, type, current_liquidity + size);
    }
    ...
    fun calculate_coin_from_lp_coin_amount_internal<CoinType>(
        ...
        //;usd value of DONK token. No not DONK, but CoinType token. 
    ...
    fun get_usd_value_internal(
        ...
        */
        ...
        //; gets value from liquidity table for the key of type
        //; i think below 2 lines are useless. 
        ...
        //;usdc_size * 10^decimals / input_size
    ...
    //@audit DDOS by adding limit orders with max quote so that list becomes too large.
    fun get_order_by_id(
        order_store: &OrderStore,
        order_id: u64
    ): (Option<Order>) {
    ...
    fun get_liquidity_size_internal(
        ...
        //;type is the Key, * will dereference and return the value stored at the key. 
